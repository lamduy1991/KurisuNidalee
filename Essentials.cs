    ...
    internal static class Essentials
    {
        ...
        internal static void SetSpells()
        {
            try
            {
                ...
                Spells["Bushwack"].SetSkillshot(0.25f, 100f, float.MaxValue, false, SkillshotType.SkillshotCircle);
        ...
        internal static void SpellsOnUpdate(EventArgs args)
        {
            ...
            SpellTimer["Bushwack"] = ((TimeStamp["Bushwack"] - Game.Time) > 0) ? (TimeStamp["Bushwack"] - Game.Time) : 0;
        ...
        private static void HeroOnCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            try
            {
                var wperlevel = new[] { 13, 12, 11, 10, 9 };
                var wcooldown = wperlevel[NotLearned(Spells["Bushwack"]) ? 1 : Spells["Bushwack"].Level - 1];
                ...
                if (sender.IsMe && args.SData.Name.ToLower() == "takedown")
                    TimeStamp["Takdown"] = Game.Time + (5 + (5 * Player.PercentCooldownMod));

                if (sender.IsMe && args.SData.Name.ToLower() == "takedown")
                    Utility.DelayAction.Add(Game.Ping + 250, Orbwalking.ResetAutoAttackTimer);

                if (sender.IsMe && args.SData.Name.ToLower() == "primalsurge")
                    TimeStamp["Primalsurg"] = Game.Time + (12 + (12 * Player.PercentCooldownMod));

                if (sender.IsMe && args.SData.Name.ToLower() == "bushwack")
                    TimeStamp["Bushwack"] = Game.Time + (wcooldown + (wcooldown * Player.PercentCooldownMod));
        ...
        internal static Dictionary<string, Spell> Spells = new Dictionary<string, Spell>
        {
            ...
            { "Javelin", new Spell(SpellSlot.Q, 1450f) },
            { "Bushwack", new Spell(SpellSlot.W, 875f) },
